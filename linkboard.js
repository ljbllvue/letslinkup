...
// FIXED: Collaboration Knowledge Pop-up Functions with proper sizing
function showCollabPopup() {
    ...
    if (collabPopup) {
        ...
        // Ensure popup content is properly sized and scrollable
        const popupContent = collabPopup.querySelector('.collab-popup-content');
        if (popupContent) {
            // Set max height to ensure it fits on screen with some margin
            const maxHeight = window.innerHeight - 40; // 40px total margin
            popupContent.style.maxHeight = maxHeight + 'px';
            popupContent.style.overflowY = 'auto';
            
            // Ensure the CTA section is always visible by checking if content overflows
            setTimeout(() => {
                const ctaSection = popupContent.querySelector('.collab-cta-section');
                if (ctaSection) {
                    // Make sure CTA section is visible
...
function redirectToCollabGuide() {
    // Navigate to resources page instead of showing alert
    window.location.href = 'ressources.html';
    closeCollabPopup();
}

// Enhanced popup initialization with responsive handling
function initializeCollabPopup() {
    // Handle window resize to adjust popup size
    window.addEventListener('resize', () => {
        const collabPopup = document.getElementById('collabPopup');
        if (collabPopup && collabPopup.style.display === 'block') {
            const popupContent = collabPopup.querySelector('.collab-popup-content');
            if (popupContent) {
...
// Add essential CSS for popup display
function addCollabPopupCSS() {
    // Check if CSS is already added
    if (document.getElementById('collab-popup-css')) return;
    
    const style = document.createElement('style');
    style.id = 'collab-popup-css';
    style.textContent = `
        .collab-popup {
            position: fixed;
            top: 0;
